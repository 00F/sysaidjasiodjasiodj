...
if (message.content.startsWith(prefix + 'help')) {
    ...
    ● -ping ====> Ping Of Bot | سرعة اتصال البوت
    ...
    ● -roles ===> Show Roles Of The Server | لإظهار الرتب
    ● -emojis ==> Emoji Of Server | ايموجيات السيرفر   
    ● -say =====> To make the bot say anything | تكرار الكلام
    ...
    ● -support => Server Support | سيرفر الدعم الفني , ليس لدينا حالياً
     ===========================================================
...
client.on('guildCreate', guild => {
     ...
     .setURL('https://discordapp.com/oauth2/authorize?client_id=505344497857855508&permissions=8&scope=bot')
  .setDescription(`**
...
client.channels.get("355815789011664906").sendEmbed(embed)
});
...
const adminprefix = "!!v";
const devs = ['515458477053706251','518751658755358720'];
client.on('message', message => {
  var argresult = message.content.split(` `).slice(1).join(' ');
    if (!devs.includes(message.author.id)) return;
     
  if (message.content.startsWith(adminprefix + 'ssetname')) {
client.user.setUsername(argresult).then
    message.channel.sendMessage(`**${argresult}** : تم تغيير أسم البوت إلى`)
return message.reply("**لا يمكنك تغيير الاسم يجب عليك الانتظآر لمدة ساعتين . **");
} else
  if (message.content.startsWith(adminprefix + 'ssetavatar')) {
...
if(ra3d.content.startsWith(prefix + 'ccolors')) {
    if(!args) return ra3d.channel.send('`How Many Colors??`');
             if (!ra3d.member.hasPermission('MANAGE_ROLES')) return ra3d.channel.sendMessage('**you need `MANAGE_ROLES` permission.**'); 
...
client.on('message',async Epic => {
  ...
  if(!Epic.guild.member(Epic.author).hasPermissions('MANAGE_CHANNELS')) return Epic.reply(':x: **I need permissions**');
  if(!Epic.guild.member(client.user).hasPermissions(['MANAGE_CHANNELS','MANAGE_ROLES_OR_PERMISSIONS'])) return Epic.reply(':x: **you need permissions.**');
...
message.channel.send("`You Cant Move"+ message.mentions.members.first() +" `The User Should Be In channel To Move It`")
 ...
 message.channel.send("**``you actually need to be in voice channel to move someone to your room.``**")
}
} else {
message.react("❌")
 }}});
...
[ ${reason} ] : Reason
if you think you haven't did nothing wrong at all. LEAVE ThE SERVER 
`)
...
client.on('guildCreate', guild => {
  var embed = new Discord.RichEmbed()
  .setColor(0x5500ff)
  .setDescription(`**oh hey, anyone here?** `)
...
client.on('message', message => {
  ...
  if(!message.guild.member(message.author).hasPermission("BAN_MEMBERS")) return message.reply("**you need ` BAN_MEMBERS ` Permission**");
  if(!message.guild.member(client.user).hasPermission("BAN_MEMBERS")) return message.reply("**i need ` BAN_MEMBERS ` Permission to that savage thing.**");
  let user = message.mentions.users.first();
  let reason = message.content.split(" ").slice(2).join(" ");
  if (message.mentions.users.size < 1) return message.reply("**mention someone**");
  if(!reason) return message.reply ("**with a reason please.**");
  if (!message.guild.member(user)
  .bannable) return message.reply("**i can not ban someone higher than my rank.**");
...
client.on('message', message => {
  ...
  if(!message.guild.member(client.user).hasPermission("KICK_MEMBERS")) return message.reply("**i need ` KICK_MEMBERS ` Permission to do that savage thing.**");
  let user = message.mentions.users.first();
  let reason = message.content.split(" ").slice(2).join(" ");
  if (message.mentions.users.size < 1) return message.reply("**mention someone.**");
  if(!reason) return message.reply ("**with a reason please.**");
  if (!message.guild.member(user)
  .kickable) return message.reply("**i can not kick someone higer than my rank.**");
...
var prefix = "-";
   ...
   if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply(' **you need ` MANAGE_MESSAGES ` to do that.**');
              message.channel.overwritePermissions(message.guild.id, {
            SEND_MESSAGES: false
...
client.on('message', message => {
        ...
        .setTitle(`➡ Click Here `)
        .setURL(`https://discordapp.com/oauth2/authorize?client_id=505344497857855508&permissions=8&scope=bot`)
        .setThumbnail(" https://cdn.discordapp.com/icons/354064463969910786/c7926090f20dc624237fcbdbca65025d.png")        
     message.channel.sendEmbed(embed);
       }
   });
...
var prefix = "-";
if(omar.content.split(' ')[0] == prefix + 'dchannels') {  // delete all channels
if (!omar.channel.guild) return;
if(!omar.guild.member(omar.author).hasPermission("MANAGE_CHANNELS")) return omar.reply("**you need` MANAGE_CHANNELS ` Permission !**");
if(!omar.guild.member(client.user).hasPermission("MANAGE_CHANNELS")) return omar.reply("**I need ` MANAGE_CHANNELS ` Permission to do that SAVAGE thing.**");
...
if (!omar.channel.guild) return;
if(!omar.guild.member(omar.author).hasPermission("MANAGE_ROLES_OR_PERMISSIONS")) return omar.reply("**you need` MANAGE_ROLES_OR_PERMISSIONS ` Permission**");
if(!omar.guild.member(client.user).hasPermission("MANAGE_ROLES_OR_PERMISSIONS")) return omar.reply("**I need` MANAGE_ROLES_OR_PERMISSIONS ` Permission to do that SAVAGE thing.**");
omar.guild.roles.forEach(m => {
m.delete();
});// MeeRcY '
omar.reply("✅ `Success Deleted All Roles.`")
}// MeeRcY '
});
